...
- **Controller**: flick *distance* is sampled from three bands (short / mid / long) — weakness-weighted but always explored, so long flicks never disappear from your diet. Target *size* then solves the model for the pace target: `W = D / (2^((T−a)/b) − 1)` with `T = max(HP-equilibrium pace, 0.93 × your recent pace)` — anchored to your own frontier so difficulty tracks you instead of pinning at a floor. Long flicks get proportionally bigger targets; the difficulty in bits stays at your edge.
